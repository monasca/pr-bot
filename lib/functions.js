...
function getRepository(name) {
  ...
  return ds.get(Repository, name).then(r => sanitizeRepository(r));
}
...
function listRepositories() {
  ...
  return ds.list(Repository).then(repos => {
    ...
    return Promise.all(repos.map(r => r.settle())).then(repos => {
...
function getRepositoryByRemote(remote) {
  ...
  return ds.list(Repository).then(repos => {
    return sanitizeRepository(repos.find(r => r.providesRemote(remote)));
  });
}
